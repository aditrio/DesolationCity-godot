...
func playerMovement():
	...
	if(Input.is_action_pressed("right") or press_right ):
	...
	elif(Input.is_action_pressed("left") or press_left ):
	...
	if(is_on_floor()):
		...
		if press_jump && can_jump == true and delay_jump :
			get_parent().get_node("timer/jump").start()
